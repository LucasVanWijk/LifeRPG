...
React + Vite + TypeScript. Data lives in the browser (`localStorage`, key `questlog:v2`); there is no backend. A new log starts empty at level 1.
...
| Path | What it holds |
| --- | --- |
| `src/domain/` | Types, quadrants and sizes, the reward formula (Main Quests ×1.5), completion/undo/level-up logic, date helpers, the design's sample data (used by tests only) |
